...
@app.route('/showq', methods=['GET', 'POST'])
def showq():
    if request.method=='POST':
        ...
        if u=='admin' and p=='secret':
            f=open('queries.txt', 'rt')
            c=f.readlines()
            #c=c.replace('\n', '<br>')
            f.close() 
            return render_template('showq.html', data=c)
    return render_template('admin.html' )
...
8)Control Systems & Simulation Lab,in association with APSSDC and SIEMENS """
        ...
        Please ensure that the query contains relevant college details so that the experts can help out better!"""
